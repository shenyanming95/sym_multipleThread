...
/**
 * CountDownLatch是通过一个计数器来实现的，构造函数需要指定计数器的初始值，
 * 每执行一次countDown()方法，计数器的值就会减1。当计数器值到达0时，它表示所有的线程已经完成了任务，然后在await()方法上等待的线程就可以恢复执行.
 * <p>
 * 用CountDownLatch模拟这样一种情况：有2个运动员在等待裁判的口令起跑，等跑到终点后告诉裁判结果，然后裁判公布结果
 * 可以使用2个CountDownLatch，初始值分别为1和2，当裁判发出起跑信号，为1的计数器-1，运动员起跑；每当一个运动员到终点，为2的计数器-1，
 * 当2个运动员一起跑到终点，裁判才可以公布成绩
 * <p>
 * Created by 沈燕明 on 2018/12/30.
 */
